            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900" x-data="{ search: '' }">
                    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                        <h3 class="text-lg font-bold">Select Project</h3>
                        ...
                        <div class="relative w-full md:w-1/3">
                            ...
                            <input type="text" x-model="search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 p-2.5" placeholder="Search project name or location...">
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col space-y-4">
                               ...
                               class="flex items-center justify-between p-5 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-50 transition w-full">
                                <div>
                                    ...
                                    <p class="font-normal text-sm text-gray-500 flex items-center gap-1">
                                        ...
                                        {{ $project->information->lokasi_project ?? 'Location not set' }}
                                    </p>
                                </div>
                                <div class="flex items-center text-indigo-600 text-sm font-semibold shrink-0">
                                    Manage Reports <span class="ml-2 text-lg">&rarr;</span>
                                </div>
                            </a>
                        @empty
                            <div class="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                <p class="text-gray-500 italic">No projects registered yet.</p>
                            </div>
                        @endforelse
                        ...
                        <div x-show="search !== '' && !Array.from($el.parentElement.children).some(el => el.tagName === 'A' && el.style.display !== 'none')" 
                            ...
                            <p class="text-gray-500 italic">No projects match the search '<span x-text="search" class="font-semibold"></span>'.</p>
                        </div>
                    </div>
                </div>
            </div>
